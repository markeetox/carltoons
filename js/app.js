...
const App = (() => {
  ...
  function boot() {
    ...
    document.getElementById("btn-discord-login").addEventListener("click", _discordLogin);
  ...
  /* ──────────────────────────────────────────────────────────
     DISCORD OAUTH LOGIN
     Flow: redirect to Discord → Discord redirects back with ?code=
           → edge function exchanges code for token + verifies role
           → we sign in with Firebase custom token
  ────────────────────────────────────────────────────────────── */
  function _discordLogin() {
  ...
  /* Call this from a separate auth/discord.html page after redirect */
  async function handleDiscordCallback(code) {
    try {
      // Exchange code for Firebase custom token via edge function
      const res = await fetch(`${API_BASE}/discord-token`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code, redirectUri: DISCORD_CONFIG.redirectUri }),
      });
      const { customToken, discordId, username, hasRole } = await res.json();

      if (!hasRole) {
        showToast("You need the Captain or Pirate role to play!");
        showScreen("login");
        return;
      }

      await auth.signInWithCustomToken(customToken);
      // onAuthStateChanged will fire and call _loadPlayer()
    } catch (err) {
  ...
  async function _renderLeaderboard() {
    ...
    snap.docs.forEach((doc, i) => {
      ...
      row.innerHTML = `
        ...
        <span class="lb-name">${d.discordId ?? "Pigeon owner"}</span>
